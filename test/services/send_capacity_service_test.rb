...
class SendCapacityServiceTest < ActiveSupport::TestCase
  ...
  #   ckb_wallet.expects(:send_capacity).with(first_pending_event.address_hash, 10000, fee: min_fee, outputs_validator: "passthrough").returns(tx_hash)
  ...
  #   min_fee = tx.fee(1000) + 100
  #   ckb_wallet.expects(:generate_tx).with(first_pending_event.address_hash, first_pending_event.capacity).returns(tx)
  #   ckb_wallet.expects(:send_capacity).with(first_pending_event.address_hash, 10000, fee: min_fee, outputs_validator: "passthrough").returns(new_tx_hash)
